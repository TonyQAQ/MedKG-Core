...
export default function Index(props) {
    ...
    const getLabelTypeList = () => {
        api.getLabelTypeList().then(res => {
            ...
            const newData = data.filter(item => item.code !== '02')
            debugger
            setLabelTypeList(newData)
            setTypeSelVal(newData.length > 0 ? data[0].code : null)
            setTypeSelVal2(newData.length > 0 ? data[0].code : null)
        }).catch(err => { })
    }
    //获取schmaList
    const getSchmaList = () => {
