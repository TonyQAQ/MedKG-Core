...
import { Select, Spin, Tooltip, Empty, Input, Modal, Button, message, Tree, Pagination, Radio, Row, Tag, Checkbox } from 'antd';
import { MyButton } from '@/components/commonWrap';
import { PlusSquareOutlined, FormOutlined, MinusCircleOutlined, TagsOutlined } from '@ant-design/icons';
...
export default function Index(props) {
    ...
    const getClassifyList = () => {
        api.getClassifyList({ taskId }).then(res => {
            const { classifyTags } = res
            setLabelTypeList(classifyTags)
            setSpinning2(false)
        }).catch(err => { })
    }
    ...
    const updateLabelAfterEdit = () => {
        ...
        newResult[labelIndex].tag = newAddLabelVal
    ...
    const tagChange = (item, tagItem, checked) => {
        setSvgSpinning(true)
        let newObj = JSON.parse(JSON.stringify(selectedTags))
        let set = new Set(newObj[item.id])
        if (set.has(tagItem.id)) {
            set.delete(tagItem.id)
        } else {
            set.add(tagItem.id)
        }
        item.tags = Array.from(set)
        api.saveSvgInfo([item], taskId, paths).then(res => {
            let newTagsObj = JSON.parse(JSON.stringify(selectedTags))
            newTagsObj[item.id] = Array.from(set)
            setSelectedTags(newTagsObj)
            setTimeout(() => {
                setSvgSpinning(false)
            }, 200);
                                                        ...
                                                        svgList.map((item, index) => {
                                                            return (
                                                                <div className='svgMainContainer' id={'svgMainContainer' + index} >
                                                                    ...
                                                                    <Row>
                                                                        {/* <Checkbox.Group defaultValue={item.tags} onChange={(e) => svgRadioChange(item, e)}>
                                                                            {
                                                                                labelTypeList.map(radioItem => {
                                                                                    return (
                                                                                        <Tooltip title={radioItem.tag}>
                                                                                            <Checkbox value={radioItem.id} option={radioItem}>{radioItem.tag}</Checkbox>
                                                                                        </Tooltip>
                                                                                    )
                                                                                })
                                                                            }
                                                                        ...
                                                                        {
                                                                            labelTypeList.map(labelItem => (
                                                                                <CheckableTag
                                                                                    key={labelItem.id}
                                                                                    checked={selectedTags[item.id].indexOf(labelItem.id) > -1}
                                                                                    onChange={svgSpinning ? null : checked => tagChange(item, labelItem, checked)}
                                                                                >
                                                                                    <span><TagsOutlined style={{ marginRight: '5px' }} />{labelItem.tag} </span>
                                                                                </CheckableTag>
                                                                            ))
                                                                        }
                                                                    </Row>
                                                ...
                                                <Row align='middle'>
                                                    {
                                                        svgList.length > 0
                                                            ...
                                                            labelTypeList.map(item => (
                                                                <CheckableTag
                                                                    key={item.id}
                                                                    checked={selectedTags[svgList[0].id].indexOf(item.id) > -1}
                                                                    onChange={checked => tagChange(svgList[0], item, checked)}
                                                                >
                                                                    <span><TagsOutlined style={{ marginRight: '5px' }} />{item.tag} </span>
                                                                </CheckableTag>
                                                            ))

                                                            // <Checkbox.Group defaultValue={svgList[0].tags} onChange={(e) => svgRadioChange(svgList[0], e)}>
                                                            //     {
                                                            //         labelTypeList.map(radioItem => {
                                                            //             return (
                                                            //                 <Tooltip title={radioItem.tag}>
                                                            //                     <Checkbox value={radioItem.id} option={radioItem}><span><TagOutlined style={{ marginRight: '5px' }} />{radioItem.category} </span></Checkbox>
                                                            //                 </Tooltip>
                                                            //             )
                                                            //         })
                                                            //     }
                                    ...
                                    labelTypeList.length > 0
                                        ...
                                        labelTypeList.map((item, index) => {
                                            return (
                                                ...
                                                >
                                                    <span className='mds-executePage-liName'>{item.tag}</span>
                                                    {
                                                        focusIndex === index
                                                            ?
                                                            <FormOutlined
