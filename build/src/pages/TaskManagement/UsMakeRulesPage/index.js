...
export default class Index extends Component {
        ...
        api.tableRules({
            ...
            this.setState({
                ...
                newColumns.map(item => {
                    ...
                    if (item.title === '过滤条件') {
                        item['width'] = 100
                    } else if (item.title === '过滤类型') {
                        item['width'] = 100
                    } else if (item.title === '统一人数') {
                        item['width'] = 100
                    } else if (item.title === '值') {
                        item['width'] = 200
                        // item['fixed'] = 'left'
                        item['render'] = (text) => {
                            return <span className={'newColumns-title'}><Tooltip title={text}>{text}</Tooltip></span>
                        }
                    }
                })
                newColumns.push({
                ...
                // 已经选择的过滤条件不一定包含所有过滤类别，因此再次判断过滤后的条件是否包含全部类别，如果不包含就进行初始化
    ...
    confirmClick = () => {
        const { taskId, litleStateCode } = this.state
        api.confirmPublish({
            taskId, litleStateCode
        }).then(res => {
            message.success(res.retmsg)
            try {
                window.opener.location.reload();
            } finally {
                api.initSchema({ taskId }).then(res => {
